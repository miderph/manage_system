Ext.define('app.view.module.contapp.ContAppController', {
...
//    	viewModel.set('cv_img_info', 1);
    	...
    	var win = this.lookupReference('cont_app_window');
    	if (win){//不重用，解决上次新建时的残留图片预览
    		//Ext.destroy( win );//有问题
    		win = null;
    	}
    	if (!win) {
            win = Ext.create('app.view.module.contapp.ContAppWindow', {
            	viewModel: viewModel
            });
...
//	    	viewModel.set('cv_img_info', img);
    ...
    showDownloadUrlList: function(btn, e) {
    	var view = this.getView();
    	var record = view.getSelectionModel().lastSelected;
		var rows = view.getSelectionModel().getSelection();// 返回值为Record数组
		...
		if(rows.length != 1) {
			...
			var win = this.lookupReference('ca_downloadurl_window');

			if (!win) {
	            win = Ext.create('app.view.module.contapp.ContAppDownloadUrlListWindow', {
            ...
            //console.log("-----:win.appGridStore=" + win.appGridStore);
	...
	recommend_sync_add: function(btn, e) {
		var rows = this.getView().getSelectionModel().getSelection();// 返回值为
		// Record
		// 数组
		if (rows.length == 0) {
    ...
    showRecommendSyncList: function(btn, e) {
    	var view = this.getView();

    	var win = this.lookupReference('ca_recommendsynclist_window');

    	if (!win) {
    		win = Ext.create('app.view.module.contapp.ContAppRecommendSyncListWindow');
    		this.getView().add(win);
    	}
...
//        win.destroy();
    ...
    onChoiceMac: function(btn, e) {
    	...
    	AppUtil.showChoiceWindow({
			view: this,
			btn: btn,
			mWin: this.lookupReference('cont_app_window'),
	...
	*/
  	contChangCmpVisible: function (cont_app_type){
		if(cont_app_type==4 || cont_app_type==13 || cont_app_type==16){
//			contZipPanel_form.show();
//			button_url_link.show();
//			videofile_grid.hide();//视频播放地址列表
//            //修改fieldLabel   
//			if (cont_app_type==16)
//			   cont_app_play_url_form.setFieldLabel('关于预览地址:');
//			else
//			   cont_app_play_url_form.setFieldLabel('静态链接地址:');
//			changestatus(true);
		}else{
...
//	 		//修改fieldLabel   
//			cont_app_play_url_form.setFieldLabel('视频播放地址:');
//			changestatus(false);
		}
  	},
