...
begin
   ...
   proc_import_sales_update_deal(p_providerId, as_providerId, v_doingStatus,v_doingStatus-3999,v_contImportStatusValue,v_tempId);
   ...
   v_tempId :=0;
   updateCount :=updateCount || ',zl_cont_sales='||v_tempId;
   v_endTime := sysdate;strTemp := '#proc_import_sales_delete_deal,end at:'||to_char(v_endTime,'yyyy-mm-dd hh24:mi:ss')||',used time='||calculate_time(v_startTime,v_endTime)||',count='||v_tempId;updateCount := updateCount || strTemp;dbms_output.put_line(strTemp);
   if v_tempId = 0 then
      null;
