...
begin
   ...
   update zl_img set UPDATE_STATUS=v_tempStatus where provider_id=p_providerId and USE_TYPE=1 and locked=0 and id in (
   ...
   loop
      update zl_img set (
            ...
            TARGET_ID,MODIFY_TIME,provider_id,2 UPDATE_STATUS
            ,icon_url1 url_icon,icon_url1 url_icon_4--,icon_url2 url_little,icon_url3 url--只更新小方图和四格图
            ,case
                when icon_url2 is not null then 0
                when icon_url3 is not null then 1
                when icon_url1 is not null then 2
             end IS_URL_USED
            from ZL_CONT_SALES_IMPORT im2 where im2.id=zl_img.id
         )
         where provider_id=p_providerId and update_status=v_tempStatus and USE_TYPE=1 and rownum<=v_block_size;
   ...
   insert into ZL_IMG(id,PLATGROUP_ID,target_id,provider_id,Use_Type,URL_ICON,URL_4_SQUARES,URL_LITTLE,URL,IS_URL_USED)
       select id,1 PLATGROUP_ID,id,provider_id,1 use_type,icon_url1 url_icon,icon_url1 url_icon_4,icon_url2 url_little,icon_url3 url
         ...
         from ZL_CONT_SALES_IMPORT im3 where im3.id in(
             select im1.id from ZL_CONT_SALES_IMPORT im1,zl_img im2
             where im1.provider_id=p_providerId and import_status=p_doingStatus and (im1.icon_url1 is not null or im1.icon_url2 is not null or im1.icon_url3 is not null) and im1.id=im2.id(+) and im2.id is null
         );
   ...
   for ii in 1..6 loop
      ...
      v_sql := 'insert into ZL_IMG(PLATGROUP_ID,target_id,provider_id,Use_Type,url)'
               ...
               ||'       and not exists(select 1 from zl_img img where img.use_type=2 and locked=1 and img.target_id=im1.id )';
   ...
   loop
      update zl_cont set (
            TYPE,NAME,CREATE_TIME,MODIFY_TIME,ACTIVE_TIME,DEACTIVE_TIME,PROVIDER_ID,UPDATE_STATUS,pinyin
         )=(select
            v_sales_type,NAME,CREATE_TIME,MODIFY_TIME,sysdate-1,sysdate+3650
            ,as_provider(p_providerId, SHOP_TYPE),2 UPDATE_STATUS,pinyin
            from ZL_CONT_SALES_IMPORT im2 where im2.id=zl_cont.id
         )
         where id in (
            select id from ZL_CONT_SALES_IMPORT im where provider_id=p_providerId and import_status=p_doingStatus
   ...
   loop
      update zl_cont_video set (
            ...
            from ZL_CONT_SALES_IMPORT im2 where im2.id=zl_cont_video.c_id
         )
         where c_id in (
            select id from ZL_CONT_SALES_IMPORT im where provider_id=p_providerId and import_status=p_doingStatus
         ) and rownum<=v_block_size and update_status=v_tempStatus;
   ...
   loop
      update ZL_CONT_SALES sales set (
            ...
            from ZL_CONT_SALES_IMPORT im2 where im2.id=sales.c_id
         )
         where c_id in (
            select id from ZL_CONT_SALES_IMPORT im where provider_id=p_providerId and import_status=p_doingStatus
         ) and rownum<=v_block_size and update_status=v_tempStatus;
