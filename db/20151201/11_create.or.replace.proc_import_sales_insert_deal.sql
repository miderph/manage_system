...
begin
   ...
   --1.1,ZL_IMG 推荐图
   loop----确保不存在重复的数据
      delete from zl_img t1 where id in (
   ...
   v_loop_count := 0;
   loop
      select nvl(min(im1.id),0) into v_startid from zl_img m1,ZL_CONT_SALES_IMPORT im1 where im1.provider_id=p_providerId and im1.import_status=p_doingStatus and (im1.icon_url1 is not null or im1.icon_url2 is not null or im1.icon_url3 is not null) and m1.id(+)=im1.id and m1.id is null;
      ...
      insert into ZL_IMG(id,PLATGROUP_ID,target_id,provider_id,Use_Type,URL_ICON,URL_4_SQUARES--,URL_LITTLE,URL
          ...
          where im3.provider_id=p_providerId and im3.import_status=p_doingStatus and (im3.icon_url1 is not null or im3.icon_url2 is not null or im3.icon_url3 is not null)
              and rownum<=v_block_size and im3.id >=v_startid;
   ...
   for ii in 1..6 loop
      v_sql := 'insert into ZL_IMG(PLATGROUP_ID,target_id,provider_id,Use_Type,url)'
               ||'   select 1 PLATGROUP_ID,c_id,provider_id,2,isp_pic_url'||ii||' from ZL_CONT_SALES_IMPORT'
   ...
   v_loop_count := 0;
   loop
      select nvl(min(im1.id),0) into v_startid from zl_cont m1,ZL_CONT_SALES_IMPORT im1 where im1.provider_id=p_providerId and im1.import_status=p_doingStatus and m1.id(+)=im1.id and m1.id is null;
      if (v_startid<=0) then--if (sql%rowcount<=0) then
         exit;
      end if;
      ...
      insert into ZL_CONT(ID,TYPE,STATUS,NAME,CREATE_TIME,MODIFY_TIME,ACTIVE_TIME,DEACTIVE_TIME,PROVIDER_ID,UPDATE_STATUS,pinyin)
         ...
         where im3.provider_id=p_providerId and im3.import_status=p_doingStatus and rownum<=v_block_size and im3.id >=v_startid
          and id in (
             select im1.id from zl_cont m1,ZL_CONT_SALES_IMPORT im1 where im1.provider_id=p_providerId and m1.id(+)=im1.id and m1.id is null
          );
   ...
   v_loop_count := 0;
   loop
      select nvl(min(im1.id),0) into v_startid from ZL_CONT_VIDEO m1,ZL_CONT_SALES_IMPORT im1 where im1.provider_id=p_providerId and im1.import_status=p_doingStatus and m1.c_id(+)=im1.id and m1.c_id is null;
      if (v_startid<=0) then--if (sql%rowcount<=0) then
         exit;
      end if;
   ...
   v_loop_count := 0;
   loop
      select nvl(min(im1.id),0) into v_startid from ZL_CONT_SALES m1,ZL_CONT_SALES_IMPORT im1 where im1.provider_id=p_providerId and im1.import_status=p_doingStatus and m1.c_id(+)=im1.id and m1.c_id is null;
      if (v_startid<=0) then--if (sql%rowcount<=0) then
         exit;
      end if;
