...
begin
   ...
   update ZL_CONT_SALES_IMPORT set import_status=0 where provider_id=p_providerId and import_status is null;
   select count(1) into v_tempId from ZL_CONT_SALES_IMPORT where update_status >= 1 and import_status=0 and provider_id= p_providerId;
   if v_tempId = 0 then
   ...
   --标记要处理的数据
   update ZL_CONT_SALES_IMPORT set import_status=1 where provider_id=p_providerId and import_status!=1 and update_status<=0;
   update ZL_CONT_SALES_IMPORT set import_status=v_prepareStatus where provider_id=p_providerId and import_status=0 and update_status>=1;
