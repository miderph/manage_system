...
begin
   ...
   --for ii in 1..2 loop
      ...
      v_sql := 'insert into ZL_IMG(PLATGROUP_ID,target_id,provider_id,Use_Type,URL_ICON,URL_4_SQUARES)'
               ||'   select 1 PLATGROUP_ID,c_id,'||as_providerId||',1,icon_url1,icon_url2 from ZL_CONT_SALES_IMPORT'
   ...
   insert into ZL_CONT_VIDEO(C_ID,NAME,ALIAS,PROVIDER_ID,UPDATE_STATUS)
      select C_ID,NAME,NAME,as_providerId,1 UPDATE_STATUS from ZL_CONT_SALES_IMPORT
         where provider_id=p_providerId and import_status=p_doingStatus;
   --ZL_CONT_SALES
   insert into ZL_CONT_SALES(C_ID,SALES_NO,PROVIDER_ID,NAME,HOT_INFO,FAKE_PRICE,SALE_PRICE,DISACCOUNT,CP_NAME,SUB_CP_NAME,PAY_TYPE_IDS,Has_Video)
      select c_id,SALES_NO,as_providerId,NAME,HOT_INFO,FAKE_PRICE,SALE_PRICE,DISACCOUNT,CP_NAME,SUB_CP_NAME,PAY_TYPE_IDS,has_video from ZL_CONT_SALES_IMPORT
         where provider_id=p_providerId and import_status=p_doingStatus;
   --恢复到p_prepareStatus
