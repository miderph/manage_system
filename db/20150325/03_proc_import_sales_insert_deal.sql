...
begin
   ...
   insert into ZL_CONT_VIDEO(C_ID,NAME,ALIAS,PROVIDER_ID,UPDATE_STATUS)
      select C_ID,NAME,NAME,as_providerId,1 UPDATE_STATUS from ZL_CONT_SALES_IMPORT
         where provider_id=p_providerId and import_status=p_doingStatus;
   --ZL_CONT_SALES
   insert into ZL_CONT_SALES(C_ID,SALES_NO,PROVIDER_ID,NAME,HOT_INFO,FAKE_PRICE,SALE_PRICE,DISACCOUNT,POST_PRICE,CP_NAME,SUB_CP_NAME,PAY_TYPE_IDS,Has_Video,URL,manage_user,audit_review,shop_type)
      select c_id,SALES_NO,as_providerId,NAME,HOT_INFO,FAKE_PRICE,SALE_PRICE,DISACCOUNT,POST_PRICE,CP_NAME,SUB_CP_NAME,PAY_TYPE_IDS,has_video,URL,manage_user,audit_review,shop_type from ZL_CONT_SALES_IMPORT
         where provider_id=p_providerId and import_status=p_doingStatus;
   --恢复到p_prepareStatus
